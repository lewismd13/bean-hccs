...
*/
	...
	if (get_property('_snojoFreeFights').to_int()<10) {
		...
		while (get_property('_snojoFreeFights').to_int()<6) {
...
*/
    ...
    while (get_property('_machineTunnelsAdv') < 5) {
		use_familiar($familiar[machine elf]);
		set_hccs_combat_mode(MODE_NULL);
		while ((available_amount($item[abstraction: thought]) == 0) && (available_amount($item[abstraction: certainty]) == 0)) {
			set_auto_attack('melfgetthought');
			adventure(1, $location[the deep machine tunnels]);
		}
		while ((available_amount($item[abstraction: thought]) == 1) && (available_amount($item[abstraction: certainty]) == 0)) {
			set_auto_attack('melfgetcertainty');
			adventure(1, $location[the deep machine tunnels]);
		}
		while ((available_amount($item[abstraction: action]) == 1) && (available_amount($item[abstraction: joy]) == 0)) {
			set_auto_attack('melfgetjoy');
			adventure(1, $location[the deep machine tunnels]);
		}
		set_auto_attack(0);
...
if (!test_done(TEST_ITEM)) {
	...
	// use abstraction: certainty if you have it. you should have it. unless you haven't coded that part yet, lazyass Manny
